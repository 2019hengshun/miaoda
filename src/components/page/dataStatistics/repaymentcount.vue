...
<el-tabs type="border-card"   v-model="activeName" @tab-click="handleClick" v-loading="lodings">
    <el-tab-pane label="线上还款列表" name="three">
        ...
        <el-table
            ...
            id="eleTable"
          >
            <el-table-column prop="userId" fixed label="ID" align="center" width="80" sortable>
            </el-table-column>  
         <el-table-column prop="userName" label="姓名" align="center" width="120" >
           ...
           <el-table-column prop="id_no" label="身份证" align="center" width="180" >
            ...
            <el-table-column prop="returnMoney" label="本息金额" align="center" width="120" >
            </el-table-column>              
            <el-table-column prop="charge_fee" label="手续费" align="center" width="120" >
              <template slot-scope="scope">
                {{ scope.row.type===1?0:scope.row.charge_fee}}
              </template>
            </el-table-column>  
            <!-- <el-table-column prop="charge_fee" label="应还款金额" align="center" width="120" >
              <template slot-scope="scope">
                {{(scope.row.realMoney + scope.row.charge_fee + scope.row.late_fee).toFixed(2) }}
              </template>
            </el-table-column>                           -->
            ...
            <el-table-column prop="late_fee" label="滞纳金" align="center" width="120" >
              <template slot-scope="scope">
                {{ scope.row.type===2?0:scope.row.late_fee}}
              </template>
            </el-table-column>  
            <el-table-column prop="penalty_money" label="逾期服务费" align="center" width="120" >
              <template slot-scope="scope">
                {{ scope.row.type===1?0:scope.row.penalty_money}}
              </template>              
            </el-table-column>   
            ...
            <el-table-column prop="status" label="还款状态" align="center" width="120"
            ...
            <el-table-column prop="type" label="还款类型" align="center" width="120" >
              <template slot-scope="scope">
                  <el-tag
                      :type="scope.row.type===1?'success':scope.row.type===2?'primary':scope.row.type===3?'danger':''"
                  >{{scope.row.type===1?'本金':scope.row.type===2?'服务费':''}}</el-tag>
              </template>  
            </el-table-column>             
    ...
    <el-tab-pane label="线下还款列表" name="four">
         ...
         <el-table-column prop="userName" label="姓名" align="center" width="120" >
           ...
           <el-table-column prop="id_no" label="身份证" align="center" width="160" >
            ...
            <el-table-column prop="ned_return_money" label="需要还款金额" align="center" width="120" >
              <template slot-scope="scope">
                {{ parseInt(scope.row.late_fee + scope.row.raise_money + scope.row.penalty_money + scope.row.charge_fee) }}
              </template>
            </el-table-column> 
            <el-table-column prop="charge_fee" label="手续费" align="center" width="120" >
            </el-table-column>              
        <el-table-column prop="late_fee" label="滞纳金" align="center" width="120" >
            ...
            <el-table-column prop="returnMoney" label="实际还款金额" align="center" width="120" >
              <template slot-scope="scope">
                {{ (scope.row.returnMoney/100).toFixed(2) }}
              </template>
            </el-table-column>   
            <!-- <el-table-column prop="raise_money" label="合同金额" align="center" width="120" >
            </el-table-column>  -->
            ...
            <el-table-column prop="returnTime" label="需要还款日" align="center" width="140" >
                <template slot-scope="scope">
                    {{scope.row.returnTime?(scope.row.returnTime|dateServer):''}}
                </template>               
            </el-table-column> 
            <el-table-column prop="trans_remark" label="描述" align="center" width="120" >
                                      <template slot-scope="scope">
                                              <el-tooltip class="item" effect="dark" :content="scope.row.trans_remark" placement="top">
                                                  <span>{{scope.row.trans_remark}}</span>
                                              </el-tooltip>
...
export default {
  ...
  methods: {
    ...
    drawLine(xData, sData, type = true) {
      console.log(xData, sData.money);
      sData.money = sData.money.map(v => (v = v / 100));
      console.log(sData);
    ...
    export2Excel2() {
      require.ensure([], () => {
        ...
        for (var i = 0; i < list.length; i++) {
          list[i].ned_return_money = parseInt(
            list[i].late_fee +
              list[i].raise_money +
              list[i].penalty_money +
              list[i].charge_fee
