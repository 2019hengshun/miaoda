<template>
    <div class="container"   >
      ...
      <div v-if="!show">
        ...
        <el-row :gutter="20" class="m20">
            <el-col :span="6" v-for="(item,i) in todayEventList3" :key="i">
                <el-card shadow="hover" :body-style="{padding: '0px'}">
                    <div :class="'grid-content grid-con-'+i">
                        <div class="grid-con-default grid-con-icon">{{item.count}}</div>
                        <div class="grid-cont-right">
    ...
    getSummary(a, b) {
      ...
      getPlatform(a, b)
        .then(response => {
          ...
          _this.todayEventList3 = [
            {
              title: "今日通过审核通过数量",
              count: data.allNowPassNumber - data.notNowPassNumber,
              percent: "",
              color: "#a7db65"
            },
            {
              title: "非今日审核通过数量",
              count: data.notNowPassNumber,
              percent: "",
              color: "#ff6c5c"
            },
            {
              title: "总共审核通过数量",
              count: data.allNowPassNumber,
              percent: "",
              color: "#a7db65"
            }
          ];
