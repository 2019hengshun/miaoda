<template>
    ...
    <el-tabs type="border-card" class="m20"  v-model="activeName" @tab-click="handleClick" v-loading="lodings">
      <el-tab-pane label="催收金额列表" name="first">         
        <el-row class="m20" >
         <el-col :span="6">
        ...
        <el-table
          ...
          >
            ...
            <el-table-column prop="allOverdueMoney" label="催收本金" align="center"   ></el-table-column>
            <el-table-column prop="all_late_fee" label="应催收违约金" align="center"  >

            </el-table-column>
            <el-table-column prop="all_charge_fee" label="全部服务费" align="center"  >

            </el-table-column>
            <el-table-column prop="all_penalty_money" label="全部逾期服务费" align="center"  >

            </el-table-column>                        
            <el-table-column prop="all_return_money" label="本息金额" align="center"  >

            </el-table-column>            
            <!-- <el-table-column prop="borrowTime" label="借款时间" align="center" width="180"  >
                <template slot-scope="scope">
                    {{scope.row.borrowTime|dateServer}}
                </template>
            </el-table-column> -->
            <el-table-column prop="borrowDay" label="催回总金额" align="center"   >
                <template slot-scope="scope">
                    <!-- {{(((scope.row.collect_money_m1+scope.row.collect_money_m2+scope.row.collect_money_m3+scope.row.collect_money_m4)/100).toFixed(2)).toFixed(2)}} -->
                    {{((scope.row.collect_money_m1+scope.row.collect_money_m2+scope.row.collect_money_m3+scope.row.collect_money_m4)/100).toFixed(2)}}
      <!-- {{(scope.row.collect_money_m1+scope.row.collect_money_m2+scope.row.collect_money_m3+scope.row.collect_money_m4)}} -->
                </template>
            </el-table-column>
            <el-table-column prop="collect_money_m1" label="已收M1" align="center"   >
               <template slot-scope="scope">
                    {{(scope.row.collect_money_m1/100).toFixed(2)}}
                </template>
            </el-table-column>
            <el-table-column prop="overdueApplyNumber_m1" label="已收M1数量" align="center"   >
            </el-table-column>            
            <el-table-column prop="collect_money_m2" label="已收M2" align="center"    >
            <template slot-scope="scope">
                    {{(scope.row.collect_money_m2/100).toFixed(2)}}
                </template>
            </el-table-column>
            <el-table-column prop="overdueApplyNumber_m2" label="已收M2数量" align="center"   >
            </el-table-column>              
            <el-table-column prop="collect_money_m3" label="已收M3" align="center"    >
                <template slot-scope="scope">
                    {{(scope.row.collect_money_m3/100).toFixed(2)}}
                </template>
            </el-table-column>
            <el-table-column prop="overdueApplyNumber_m3" label="已收M3数量" align="center"   >
            </el-table-column>              
            <el-table-column prop="collect_money_m4" label="已收M3+" align="center"    >
                <template slot-scope="scope">
                    {{(scope.row.collect_money_m4/100).toFixed(2)}}
                </template>
            </el-table-column>
            <el-table-column prop="overdueApplyNumber_m4" label="已收M3+数量" align="center"   >
            </el-table-column>              
            <!-- <el-table-column prop="overdue_day" label="逾期天数" align="center"   ></el-table-column>
            <el-table-column prop="late_fee" label="逾期金" align="center"   ></el-table-column>
            <el-table-column prop="ned_return_money" label="需要还钱数" align="center"   ></el-table-column>
      ...
      <el-tab-pane label="催收列表成功展示" name="second"> 
        <el-row class="m20" >
            ...
            id="eleTable"
          >
             <el-table-column prop="userId" fixed label="ID号" align="center" width="80" >
            </el-table-column> 
             ...
             <el-table-column prop="createTime" label="实际还款日" align="center" width="140">
       <template slot-scope="scope">
                    {{scope.row.createTime|dateServer}}
                </template>   
            </el-table-column> 
             <el-table-column prop="raise_money" label="提现金额" align="center" width="100"  >
            </el-table-column> 
            <el-table-column prop="ned_return_money" label="本息金额" align="center" width="100"  >
            </el-table-column> 
            <el-table-column prop="charge_fee" label="服务费" align="center" width="100"  >
            </el-table-column>                         
             <el-table-column prop="late_fee" label="滞纳金" align="center" width="120"  >
            </el-table-column> 
             <el-table-column prop="penalty_money" label="逾期服务费" align="center" width="120"  >
            </el-table-column>             
             <el-table-column prop="overdue_day" label="逾期天数" align="center" width="100"  >
            </el-table-column>  
             ...
             <el-table-column prop="realMoney" label="实际还款金额" align="center" width="140"  >
                  <template slot-scope="scope">
                    {{(scope.row.realMoney/100).toFixed(2)}}
                </template>    
            </el-table-column>  
             <!-- <el-table-column prop="id" label="实际还款违约金" align="center" width="80"  >
            </el-table-column>    -->
             <el-table-column prop="reduceMoney" label="减免金额" align="center" min-width="140"  >
            </el-table-column>                                                                                                                                     
          </el-table>  
            <el-row class="m20" v-if="total>0">
                 <div style="float:right">
    ...
    export2Excel() {
      require.ensure([], () => {
        ...
        const filterVal = [
          ...
          "borrowDay",
          "collect_money_m1",
          "overdueApplyNumber_m1",
          "collect_money_m2",
          "overdueApplyNumber_m2",
          "collect_money_m3",
          "overdueApplyNumber_m3",
          "collect_money_m4",
          "overdueApplyNumber_m4"
        ];
        let list = JSON.parse(JSON.stringify(this.tableData));

        for (var i = 0; i < list.length; i++) {
          list[i].i = i + 1;
          list[i].collect_money_m1 = (list[i].collect_money_m1 / 100).toFixed(
            2
          );
          list[i].collect_money_m2 = (list[i].collect_money_m2 / 100).toFixed(
            2
          );
          list[i].collect_money_m3 = (list[i].collect_money_m3 / 100).toFixed(
            2
          );
          list[i].collect_money_m4 = (list[i].collect_money_m4 / 100).toFixed(
            2
          );
          list[i].borrowDay = (
            (Number(list[i].collect_money_m1) +
              Number(list[i].collect_money_m2) +
              Number(list[i].collect_money_m3) +
              Number(list[i].collect_money_m4)) /
            100
          ).toFixed(2);
        }
    ...
    export2Excel2() {
      require.ensure([], () => {
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          list[i].realMoney = (list[i].realMoney / 100).toFixed(2);
        }
        const data = this.formatJson(filterVal, list);
